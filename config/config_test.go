...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{"profiling disabled skips checks", func(c *Config) { c.Profiling.Enabled = false; c.Profiling.Endpoint = "" }, false},
...
func TestBuildDSN(t *testing.T) {
	db := &DatabaseConfig{Host: "localhost", Port: "5432", Name: "auth", User: "auth", Password: "secret", SSLMode: "disable"}
	got := db.BuildDSN()
	want := "postgresql://auth:secret@localhost:5432/auth?sslmode=disable"
	if got != want {
...
func TestEnvHelpers(t *testing.T) {
	...
	// varied keys AND defaults (not just the production constants) so every
	// branch is covered and the params are genuinely tested — this also keeps
	...
	if getEnvDurationSecondsWithMax("MAX_UNSET", 5, 30) != 5 {
		t.Error("getEnvDurationSecondsWithMax: unset should return default")
	}
	t.Setenv("MAX_VALID", "15s")
	if getEnvDurationSecondsWithMax("MAX_VALID", 4, 30) != 15 {
		t.Error("getEnvDurationSecondsWithMax: valid value should parse")
	}
	t.Setenv("MAX_BAD", "bad")
	if getEnvDurationSecondsWithMax("MAX_BAD", 6, 20) != 6 {
		t.Error("getEnvDurationSecondsWithMax: invalid value should return default")
	}
	t.Setenv("MAX_OVER", "99s")
	if getEnvDurationSecondsWithMax("MAX_OVER", 2, 30) != 2 {
